...
function allPrice(dataList) {

  return dataList.reduce((acc, el) => (acc += el.price), 0);
}
allPrice(dataList);

// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price)
function lowPrice(dataList) {
//   for (let i = 0; i < dataList.length; i++) {
//     if (dataList[i].price < dataList[i].old_price) count += 1;
//   }
let count = dataList.reduce((acc, { price, old_price }) => {
  if (price < old_price) {
    acc++;
  }
  return acc;
...
console.log(lowPrice(dataList));

// 3) Товари, які доступні (sell_status:"available")
function availableItems(dataList){
    let statusAvailble = [];
    statusAvailble = dataList.filter(
      ({ sell_status }) => sell_status === "available"
    );
    return statusAvailble;
}
...
function avaibleObjItems(){
    let availableItemsList = availableItems(dataList);
    let newObj = [];
    newObj = availableItemsList.map(({ id, price, old_price, usd_price }) => ({
      id,
      price,
      old_price,
      usd_price,
